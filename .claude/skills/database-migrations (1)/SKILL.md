...
Use **Alembic** for managing PostgreSQL schema migrations. Migrations are version-controlled Python scripts that track database changes over time.
...
Output:
```
Current revision for postgresql://user:pass@localhost/mydb: 20250115_143022
```
...
1. **Backup database:**
   ```bash
   pg_dump -U postgres mydb_prod > backup.sql
   ```

2. **Test migration on copy:**
   ```bash
   # Restore backup to test database
   psql mydb_test < backup.sql
