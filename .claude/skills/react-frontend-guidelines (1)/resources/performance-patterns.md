...
// Component memoization - re-renders only when props change
...
export const ExpensiveComponent: FC<{ items: Item[] }> = ({ items }) => {
  // Memoize calculations - recalculates only when items change
  const sortedItems = useMemo(() => {
    console.log('Sorting...');
    return [...items].sort((a, b) => a.title.localeCompare(b.title));
  }, [items]);

  // Memoize callback - stable reference between renders
...
export const VirtualList: FC<{ items: Item[] }> = ({ items }) => {
  ...
  const virtualizer = useVirtualizer({
    ...
    overscan: 5, // Number of items beyond viewport
  });
