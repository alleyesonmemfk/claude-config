...
Support OAuth 2.0 authentication with **Google** and **Telegram**, allowing users to sign in with their social accounts and link multiple OAuth providers to a single account.
...
curl "http://localhost:4000/api/v2/auth/oauth/google/callback?code=test&state=test"
```
...
- Never log access tokens (they grant account access)
